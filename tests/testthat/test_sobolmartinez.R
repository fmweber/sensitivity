...
test_that("Models with matrix output work correctly", {
  ...
  x_onecol <- sobolmartinez(model = sobol.fun_onecol, X1, X2)
  ...
  x_onecol_onepar <- sobolmartinez(model = sobol.fun_onecol_onepar, 
                                   X1_onepar, X2_onepar)
  
  # Trying to perform a bootstrap should fail:
  expect_error(sobolmartinez(model = sobol.fun_matrix, X1, X2, nboot = 100),
...
test_that("Models with array output work correctly", {
  ...
  x_onedim3 <- sobolmartinez(model = sobol.fun_onedim3, X1, X2)
  ...
  x_onecol_onedim3 <- sobolmartinez(model = sobol.fun_onecol_onedim3, X1, X2)
  ...
  X1_onepar <- data.frame(X1 = matrix(runif(1 * n), nrow = n))
  X2_onepar <- data.frame(X1 = matrix(runif(1 * n), nrow = n))
  x_onecol_onedim3_onepar <- sobolmartinez(model = 
  ...
  expect_error(sobolmartinez(model = sobol.fun_dim4, X1, X2),
               "If the model returns an array, it must not have")
  
  # A model function returning a list should throw an error:
  expect_error(sobolmartinez(model = function(X) list(sobol.fun(X)), X1, X2))
  
  # Trying to perform a bootstrap should fail:
  expect_error(sobolmartinez(model = sobol.fun_array, X1, X2, nboot = 100),
